...
def update_coef(new_s, new_a, s, a, c, theta, r):
    ...
    for i in xrange(N):
        deriv = math.cos(math.pi * np.dot(c[i], x))
...
def main():
    ...
    c = get_c(3, basis+1)
    ser = serial.Serial('/dev/ttyACM0', 115200) # opens line of communication with Serial on highest freq... .usbserial if it doesn't work
    print "setup done"
