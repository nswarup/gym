# this is gym file with 2 value functions
import random
import math
import numpy as np
import gym
...
def best_action(s, c, theta1, theta2):
    """
    :param s: list of state variables
    :param c: list of N/2 vectors of dimension 5
    :param theta: vector of dimension N
    ...
    """
    left_val = q_value(s, c, theta1)
    right_val = q_value(s, c, theta2)
...
def update_coef(new_s, s, c, theta, theta_next, r):
    ...
    # for i in xrange(N):
    #     deriv = math.cos(math.pi * np.dot(c[i], s))
    #     theta_new[i] = min(max(0, theta[i] + temp * deriv), 100)
    return theta_new
...
def main():
    ...
    theta1 = [random.random() for _ in xrange(N)]
    theta2 = [random.random() for _ in xrange(N)]
    c = get_c(4, basis+1)
    print "setup done"
    ...
    for episode_num in xrange(2000):
        ...
        last_action = best_action(last_state, c, theta1, theta2)
        total_reward = 0
        done = False
        while not done:
            state, rew, done, _ = env.step(last_action)
            reward = get_reward(state)
            state = normalize(state)
            action = best_action(state, c, theta1, theta2)
            ...
            if (action == 0):
            	...
            	theta_next = theta2
            if (last_action == 0):
            	theta1 = update_coef(state, last_state, c, theta1, theta_next, reward)
            else:
            	theta2 = update_coef(state, last_state, c, theta2, theta_next, reward)
